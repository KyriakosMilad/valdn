...
func Test_arrayRule(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "test array rule with non-array value",
			args: args{
				fieldName:  "arrayField",
				fieldValue: 1,
				ruleValue:  "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err, validationErr := arrayRule(tt.args.fieldName, tt.args.fieldValue, tt.args.ruleValue)
			if (err != nil) != tt.wantErr {
				t.Errorf("arrayRule() got = %v, want %v", err, tt.wantErr)
			}
			if (validationErr != "") != tt.wantValidationErr {
				t.Errorf("arrayRule() got = %v, want %v", validationErr, tt.wantValidationErr)
			}
		})
	}
}
...
func Test_structRule(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "test struct rule with non-struct value",
			args: args{
				fieldName:  "structField",
				fieldValue: 1,
				ruleValue:  "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err, validationErr := structRule(tt.args.fieldName, tt.args.fieldValue, tt.args.ruleValue)
			if (err != nil) != tt.wantErr {
				t.Errorf("structRule() got = %v, want %v", err, tt.wantErr)
			}
			if (validationErr != "") != tt.wantValidationErr {
				t.Errorf("structRule() got = %v, want %v", validationErr, tt.wantValidationErr)
			}
		})
	}
}
